...
function initializeChapterSelection() {
    ...
    const topicGroups = [
        {
            title: 'Chapter 4: Vīthisaṅgaha',
            icon: '📘',
            chapters: allChapters.filter(c => c.includes('Vīthisaṅgaha'))
        },
        {
            title: 'Chapter 5: Vīthimuttasaṅgaha',
            icon: '📗',
            chapters: allChapters.filter(c => c.includes('Vīthimuttasaṅgaha'))
        },
        {
            title: 'Additional Questions (Day 1 & 2)',
            icon: '📝',
            chapters: allChapters.filter(c => c.includes('Additional Exams'))
        },
        {
            title: 'Day 3 (Oral Exam)',
            icon: '🎤',
            chapters: allChapters.filter(c => c.includes('Oral'))
        }
    ];

    topicGroups.forEach(group => {
        if (group.chapters.length === 0) return;
        createTopicSection(container, group.title, group.icon, group.chapters);
    });
}
...
function createTopicSection(container, title, icon, chapters) {
    ...
    chapters.forEach(chapter => {
        ...
        const shortName = chapter
            .replace('Vīthisaṅgaha (Chapter 4) ', '')
            .replace('Vīthimuttasaṅgaha (Chapter 5) ', '')
            .replace('Additional Exams ', '')
            .replace('Additional ', '');
...
function initializeBrowseFilter() {
    ...
    const optgroups = {
        'Chapter 4: Vīthisaṅgaha': allChapters.filter(ch => ch.includes('Vīthisaṅgaha')),
        'Chapter 5: Vīthimuttasaṅgaha': allChapters.filter(ch => ch.includes('Vīthimuttasaṅgaha')),
        'Additional Questions (Day 1 & 2)': allChapters.filter(ch => ch.includes('Additional Exams')),
        'Day 3 (Oral Exam)': allChapters.filter(ch => ch.includes('Oral'))
    };

    Object.entries(optgroups).forEach(([label, chapters]) => {
...
function displayQuestion() {
    ...
    const shortChapter = question.chapter
        .replace('Vīthisaṅgaha (Chapter 4) ', 'VS ')
        .replace('Vīthimuttasaṅgaha (Chapter 5) ', 'VM ')
        .replace('Additional Exams ', 'Additional Questions ')
        .replace('Additional ', '');
    const displayNumber = question.sourceNumber && question.sourceNumber.length > 0
        ? question.sourceNumber
        : question.id;
    
    // Update question number, chapter and text
    document.getElementById('q-number').textContent = `Q${displayNumber}`;
...
function displayReviewList(filter) {
    ...
    currentQuiz.questions.forEach((question, index) => {
        ...
        item.innerHTML = `
            ...
            <div class="review-question"><strong>Q${displayNumber}:</strong> ${question.question}</div>
    ...
    toDisplay.forEach(question => {
        ...
        item.innerHTML = `
            <div class="browse-question">
                <span class="browse-q-num">Q${displayNumber}</span>
